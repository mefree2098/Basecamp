...
- Promoted lightweight founder registration into a shared auth/session layer with cookie-backed sessions, site/Google/Microsoft/Meta account creation, provider avatar support, a top-right profile menu, a profile/settings page, notifications, and wizard resume prompts for saved AI conversations.
...
- Playwright QA on `/wizard` and `/profile` confirms provider registration buttons, profile-picture avatar display, top-right account menu, notification menu, hidden register fields for signed-in users, and saved-conversation resume prompts.
